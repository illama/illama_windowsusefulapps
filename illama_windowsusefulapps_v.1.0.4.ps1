...
function Show-TaskSchedulerMenu {
    ...
    Write-Host "2. Ajouter une application au démarrage" -ForegroundColor Green
    Write-Host "3. Activer une application" -ForegroundColor Green
    Write-Host "4. Désactiver une application" -ForegroundColor Yellow
    Write-Host "5. Désactiver plusieurs applications" -ForegroundColor Yellow
...
function Disable-MultipleStartupApps {
    ...
    Write-Host "`n========================================================" -ForegroundColor Gray
    Write-Host "Entrez les numéros des applications à désactiver" -ForegroundColor Yellow
    ...
    $input = Read-Host "`nVotre sélection"
    ...
    # Support pour les listes séparées par virgule ou espace
    else {
    ...
    if ($selectedIndices.Count -eq 0) {
        Write-Host "Aucune sélection valide." -ForegroundColor Red
        return
    }
    ...
    if ($selectedIndices.Count -eq 0) {
        Write-Host "Aucun numéro valide dans la sélection." -ForegroundColor Red
        return
    }
    
    # Afficher un résumé avant confirmation
    Write-Host "`n========================================================" -ForegroundColor Gray
    Write-Host "Applications sélectionnées pour désactivation:" -ForegroundColor Yellow
    ...
    Write-Host "`n========================================================" -ForegroundColor Gray
    $confirm = Read-Host "Confirmer la désactivation de ces $($selectedIndices.Count) application(s)? (O/N)"
    ...
    # Désactivation des applications sélectionnées
    Write-Host "`nDésactivation en cours..." -ForegroundColor Cyan
    $successCount = 0
    $errorCount = 0
    ...
    foreach ($idx in $selectedIndices) {
        ...
        try {
            if ($selectedApp.Type -eq "Tâche planifiée") {
                Disable-ScheduledTask -TaskName $selectedApp.Nom -TaskPath $selectedApp.Emplacement -ErrorAction Stop
                Write-Host "[OK] $($selectedApp.Nom) désactivée" -ForegroundColor Green
                $successCount++
            }
            elseif ($selectedApp.Type -eq "Registre") {
                Remove-ItemProperty -Path $selectedApp.Emplacement -Name $selectedApp.Nom -ErrorAction Stop
    ...
    if ($errorCount -gt 0) {
        Write-Host "  Échouées : $errorCount" -ForegroundColor Red
    }
    Write-Host "========================================================" -ForegroundColor Gray
}
...
function Start-LanguageManager {
    ...
    try {
        ...
        try {
            ...
            Write-Host "  OK ctfmon.exe ajouté au démarrage automatique" -ForegroundColor Green
        } catch {
            Write-Host "  X Erreur ctfmon: $($_.Exception.Message)" -ForegroundColor Red
        }
        
        Write-Host ""
        Write-Host "[4/4] Désactivation synchronisation..." -ForegroundColor Green
